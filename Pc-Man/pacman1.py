...
player = Actor("pacman_o")  # type: ignore
SPEED = 3

def draw():  # Pygame Zero draw function
    global pacDots, player
    screen.blit('header', (0, 0))  # type: ignore
    screen.blit('colourmap', (0, 80))  # type: ignore
    pacDotsLeft = 0
    ...
    player.draw()
    if player.status == 1:
        screen.draw.text("GAME OVER" , center=(300, 434), owidth=0.5, ocolor=(255,255,255), color=(255,64,0) , fontsize=40)  # type: ignore
    if player.status == 2:
        screen.draw.text("YOU WIN!" , center=(300, 434), owidth=0.5, ocolor=(255,255,255), color=(255,64,0) , fontsize=40)  # type: ignore
    ...
    if player.status == 0:
        ...
        if player.inputActive:
            ...
            if player.movex or player.movey:
                inputLock()  # type: ignore
                animate(player, pos=(player.x + player.movex, player.y + player.movey), duration=1/SPEED, tween='linear', on_finished=inputUnLock)  # type: ignore
        ...
        if (dirs[ghosts[g].dir] == 0 or randint(0,50) == 0):
            ...
            while d == -1:
                ...
                if dirs[rd] == 1:
                    d = rd
            ghosts[g].dir = d
        animate(ghosts[g], pos=(ghosts[g].x + dmoves[ghosts[g].dir][0]*20, ghosts[g].y + dmoves[ghosts[g].dir][1]*20), duration=1/SPEED, tween='linear', on_finished=flagMoveGhosts)  # type: ignore
    ...
    while x < 30:
        ...
        while y < 29:
            if gamemaps.checkDotPoint(10+x*20, 10+y*20):
                pacDots.append(Actor("dot",(10+x*20, 90+y*20)))  # type: ignore
                pacDots[a].status = 0
                a += 1
            y += 1
        x += 1
...
def initGhosts():
    ...
    g = 0
    while g < 4:
        ghosts.append(Actor("ghost"+str(g+1),(270+(g*20), 370)))  # type: ignore 
